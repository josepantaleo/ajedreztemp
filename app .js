      ...
      // interval liviano (cada 15s) que solo refresca el cartel de "esperando
      // a X, WO automático en Y min" dentro de la partida abierta. Se
      // necesita aparte del reloj visual porque ese solo corre en partidas
      // con reloj (ver updateTournamentClockDisplay/tournamentClockTimer),
      // y este aviso de WO tiene que actualizarse también en partidas sin
      // reloj configurado.
      let tournamentWOStatusTimer = null;
    ...
    <tbody>${rowsHtml}</tbody>
      ...
      function updateTournamentMatchBar(gameRow) {
        ...
        if (gameRow && gameRow.status === "finished") {
          ...
          clearInterval(tournamentWOStatusTimer);
        ...
        // Si falta que entre uno de los dos jugadores y el torneo tiene
        // configurado un "tiempo reglamentario de espera", calculamos cuánto
        // falta para el WO automático (misma cuenta que ya se mostraba en la
        // lista de mesas de la ronda, ver renderTournamentPairings más
        // arriba, pero ahora visible también acá adentro, en la partida
        // abierta). Aplica con o sin reloj: el WO automático no depende de
        // que el torneo tenga tiempo configurado.
        const joined = (gameRow && gameRow.joined) || { w: false, b: false };
        const onlyOneJoined = gameRow && gameRow.status === "ongoing" && joined.w !== joined.b;
        const graceMinutes = (lastTournamentState && lastTournamentState.meta && Number(lastTournamentState.meta.woGraceMinutes)) || 0;
        let woEtaText = "";
        if (onlyOneJoined && graceMinutes > 0 && gameRow.startedAt) {
          const remainingMs = gameRow.startedAt + graceMinutes * 60000 - Date.now();
          const absentName = joined.w ? tournamentMatchCtx.blackName : tournamentMatchCtx.whiteName;
          woEtaText =
            remainingMs > 0
              ? `⏱️ Esperando a ${absentName} — WO automático en ${Math.ceil(remainingMs / 60000)} min.`
              : `⏱️ Se cumplió el tiempo de espera reglamentario para ${absentName}.`;
        }

        if (tournamentClockWaitingForBothPlayers()) {
          const missing = !joined.w ? tournamentMatchCtx.whiteName : tournamentMatchCtx.blackName;
          statusEl.textContent = woEtaText || `⏳ Esperando a que entre ${missing}. El reloj arranca recién con la primera jugada.`;
        } else if (woEtaText) {
          statusEl.textContent = woEtaText;
        } else {
      ...
      async function enterTournamentMatch(round, board, whiteName, blackName, whiteEmail, blackEmail) {
        try {
          ...
          // Refresca el cartel de "WO automático en X min" cada 15s aunque
          // el torneo no tenga reloj (ver tournamentWOStatusTimer arriba).
          clearInterval(tournamentWOStatusTimer);
          tournamentWOStatusTimer = setInterval(() => updateTournamentMatchBar(tournamentCurrentGameRow), 15000);
          ...
          if (!myColor) {
            ...
            // Avisa que este jugador ya está presente. En partidas con reloj
            // esto además evita que se pueda mover hasta que los dos
            // entraron (ver tournamentClockWaitingForBothPlayers), así
            // ninguno pierde tiempo de reloj por no haber llegado todavía.
            // Se llama siempre, tenga o no tenga reloj el torneo, porque
            // fbAutoDeclareForfeits usa esta misma marca de presencia para
            // el WO automático por tiempo de espera reglamentario (si esto
            // solo se llamara con reloj, un torneo "sin reloj" nunca
            // registraría que alguien entró y el WO automático no se
            // dispararía nunca).
